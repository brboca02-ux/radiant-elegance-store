  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const catM = await ensureCategory("Masculino", "masculino");
        const catF = await ensureCategory("Feminino", "feminino");
        const catP = await ensureCategory("Plus Size", "plus-size");

        const CATEGORIES = { masculino: catM, feminino: catF, plus_size: catP };
        ...
        for (const p of products) {
          ...
          if (pErr) {
            results.push({ name: p.name, error: pErr.message });
            continue;
          }

          const variants = p.sizes.map(s => ({ product_id: prod.id, size: String(s), color: "Padrão", stock: 10 }));
          await supabase.from("product_variants").insert(variants);
          if (p.img) await supabase.from("product_images").insert({ product_id: prod.id, url: p.img, position: 0, is_primary: true });
