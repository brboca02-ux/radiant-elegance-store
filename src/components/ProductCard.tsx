  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-1 flex-col text-center ${size === "compact" ? "pt-2" : "pt-3"}`}>
        <h3 className={`font-display leading-snug line-clamp-2 overflow-hidden text-ellipsis ${size === "compact" ? "text-[10px] md:text-[11px] min-h-[2.4em]" : "text-[12px] sm:text-sm min-h-[2.6em]"}`}>
          {product.node.title}
        </h3>
        <p className={`${size === "compact" ? "text-[10px] md:text-xs pt-1" : "text-xs sm:text-sm pt-2"} mt-auto font-semibold text-foreground`}>
